...
function BlogEdit() {
  ...
  const handleButton = async () => {
    ...
    if (!title.trim() && !description.trim()) {
      setMessage({ text: "Please enter Something to edit", type: 'error' });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!token) {
        setMessage({ text: "You must be logged in to edit a blog", type: 'error' });
        setLoading(false);
        return;
      }
      ...
      setTitle("");
      setDescription("");
      setMessage({ text: "Blog Edited successfully!", type: 'success' });
    }
    catch(e: any) {
      console.error("Error editing blog:", e);
      setMessage({
        text: e.response?.data?.message || "Unable to edit the blog. Please try again later.",
        type: 'error'
      });
    }
    finally {
      ...
      <div className="max-w-screen min-h-screen text-white flex flex-col bg-black">
        ...
        <div className="w-[85%] md:w-[65%] rounded-lg h-auto mx-auto shadow-2xl shadow-blue-500/20 border border-gray-800 justify-center mt-25 md:my-auto p-5 md:p-7">
          <div>
            ...
            <div className="text-sm text-gray-300 mt-3">Refine your thoughts, update your stories, and perfect your ideas before sharing them with the world. Every edit brings your voice closer to clarity.</div>
          </div>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? "Editing the Blog..." : "Edit Blog"}
            <BottomGradient />
          </button>
        </div>
      </div>
